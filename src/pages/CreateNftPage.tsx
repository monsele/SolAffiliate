...
import { Metaplex, walletAdapterIdentity, pinataStorage, toMetaplexFileFromBrowser } from '@metaplex-foundation/js'; // Added pinataStorage
import { PublicKey, clusterApiUrl } from '@solana/web3.js';
import { motion } from 'framer-motion';
import { LinkIcon, AlertTriangleIcon } from 'lucide-react'; // Added AlertTriangleIcon
...
const CreateNftPage: React.FC = () => {
  ...
  const metaplex = useMemo(() => {
    if (wallet.connected && wallet.publicKey) {
      // Use Pinata for storage.
      // IMPORTANT: Replace 'YOUR_PINATA_JWT_PLACEHOLDER' with your actual Pinata JWT.
      return Metaplex.make(connection)
        .use(walletAdapterIdentity(wallet))
        .use(pinataStorage({ pinataJwt: 'YOUR_PINATA_JWT_PLACEHOLDER', gateway: 'https://gateway.pinata.cloud' }));
    }
  ...
  const handleCreateNFT = async () => {
    ...
    try {
      ...
      setMessage({
        type: 'success',
        text: `NFT minted successfully! Mint Address: ${nft.address.toBase58()}. View on Explorer: https://explorer.solana.com/address/${nft.address.toBase58()}`,
      });
  ...
  return (
    <div className="space-y-8 p-6 bg-gray-900 text-white rounded-lg shadow-xl max-w-2xl mx-auto">
      <div className="p-4 rounded-lg bg-yellow-500/20 text-yellow-300 border border-yellow-600 flex items-start space-x-3">
        <AlertTriangleIcon className="h-5 w-5 text-yellow-400 flex-shrink-0 mt-0.5" />
        <div>
          <h3 className="font-semibold">Developer Note: Pinata Configuration Required</h3>
          <p className="text-sm">
            This page is configured to use Pinata for NFT metadata storage.
            To enable minting, you must replace <code>'YOUR_PINATA_JWT_PLACEHOLDER'</code> in the
            <code>src/pages/CreateNftPage.tsx</code> file with your actual Pinata JWT.
          </p>
        </div>
      </div>
      <h1 className="text-3xl font-bold text-center">Create Your NFT</h1>
      <p className="text-gray-400 text-center">
        {wallet.connected ? (
          <>
            ...
            <br />
            Ensure you have enough SOL on the connected network for transaction fees. Pinata will be used for storage.
          </>
        ) : (
